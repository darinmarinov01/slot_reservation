...
const DeleteSlotModal = ({ data, isBookedSlot = true, onClose, onConfirm }: Props) => {
    const { setAlert } = useAlert()
    const handleDeleteSlot = async (event: SelectChangeEvent | any) => {
        event.preventDefault()
        if (data?.id) {
            try {
                const requestedUrl = isBookedSlot ? '/api/bookedSlots/deleteSlot' : '/api/slots/deleteSlot'
                await fetch(requestedUrl, {
                    body: JSON.stringify({...data}),
                    headers: headers,
                    method: 'DELETE',
                }).then(async response => {
                    setAlert('', `${'Booked Slot is removed'}`, 'success')
                    if (!isBookedSlot && response.ok) {
                        await fetch('/api/bookedSlots/deleteSlots', { // if slot is deleted, remove all booked slots connected with him
                            body: JSON.stringify({...data}),
                            headers: headers,
                            method: 'DELETE',
                        })
